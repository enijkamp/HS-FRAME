 ...
 *
 * Usage:
 *    S3Map = mexc_ComputeSUM3( M2Map, S3Template, subsampleS3, nS2Trans );
 *
 *eg. tmpS3 = mexc_ComputeSUM3( tmpM2(:), S3T(r), 1, numPartRotate);
 *
...
void compute()
{
    ...
    for( iT = 0; iT < nS3Template; ++iT )
    {
        ...
        f2 = mxGetField( pA, 0, "selectedLogZ" );
	    if ( f2 == NULL )
	    {
	      mexErrMsgTxt( "no such field 6 in struct" );
	    }
        datatype = mxGetClassID(f2);
        if (datatype != mxSINGLE_CLASS)
        mexErrMsgTxt("warning !! single precision required.");
      
        selectedLogZ = (const float*)mxGetPr(f2);
