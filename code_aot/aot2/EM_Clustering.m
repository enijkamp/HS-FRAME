...
if loadInitialization
    file_loaded = load([imageFolder '/initialization/weightingMatrix' int2str(seed) '.mat']);            
    MAX3scoreAll = file_loaded.MAX2scoreAll;            
else
...
for it = 1:numIteration
    ...
    for c = 1:NumCluster
        ...
        clusters{c}.imageIdx = find(ImageCluster==c);
        clusters{c}.numImage = size(clusters{c}.imageIdx, 1);
        if clusters{c}.numImage>numImage*0.9,isabnormal = true;end
    end
    if isabnormal
        disp('abnormal!!!!!!!!!!!!!');
        MAX3scoreAll = rand(numImage, NumCluster);
        for c = 1:NumCluster
            [~, ImageCluster] = max(MAX3scoreAll, [], 2);
            clusters{c}.imageIdx = find(ImageCluster==c);
            clusters{c}.numImage = size(clusters{c}.imageIdx, 1);
    ...
    [purity(it), entropy(it)] = evaluateClustering(imageFolder, 'gt.txt', MAX3scoreAll);
    
    %save(sprintf('working/aveMAX2_iter%d.mat',it-1),'aveMAX2','PartOnOff');
    ...
    purity
    entropy
    save( sprintf('working/partModel_iter%d.mat',it), 'clusters');
