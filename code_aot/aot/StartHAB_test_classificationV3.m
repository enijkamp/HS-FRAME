...
%         imageName = dir([imageFolder '/*.jpg']);
        ...
        for img= 1:length(imgList)   
            ...
            tmpIm = imread(fullfile(imgPath, categoryName,imgList(img).name));
            ...
            disp(sprintf('Finish image %d : %.2f second.', img, toc));
        end
        switch FeatureTypeName
            case 'train'
                trainFeatures = SPM_SUM3;
                save(['../output/trainFeatures_' categoryNames{iClass} '.mat'], '-v7.3', 'trainFeatures');
            otherwise
                testFeatures = SPM_SUM3;
                save(['../output/testFeatures_' categoryNames{iClass} '.mat'], '-v7.3', 'testFeatures');
        end
    end 
end
