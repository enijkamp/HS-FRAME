...
if read_feature_from_data && exist(['./output/' para.name '/HAB_All_Features.mat'],'file')
    ...
    for iClass = para.task_ids
        load(['output/trainFeatures_' categoryNames{iClass} '.mat']);
        load(['output/testFeatures_' categoryNames{iClass} '.mat']);
    ...
    save('./output/HAB_All_Features.mat', '-v7.3', 'trainFeatures', 'testFeatures', 'trainLabels', 'testLabels');
end
...
if run_svm_params
    ...
    save('output/svmResults.mat', 'svmResults');
...
if run_svm
    ...
    save('output/svm_accuracy.mat', 'acc');
    save('output/svm_model.mat', 'model');
end
...
if run_confusion
    ...
    print(h, '-dpdf', 'output/confusion.pdf');
    saveas(h, 'output/confusion.png', 'png');
